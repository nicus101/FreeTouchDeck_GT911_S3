...
// (ESP32 S3 compatible GT911 driver)
#define GT911_S3
...
//------------ Settings for the GT911 ESP32 S3 driver ------------
#ifdef GT911_S3
#define INT_PIN 16
#define RST_PIN 15
#endif
...
void touchStart() {
 if (ts.begin(INT_PIN, RST_PIN,GT911_I2C_ADDR_BA)!=true) {
  ...
  Serial.print(ts.i2cAddr, HEX);

  Wire.beginTransmission(ts.i2cAddr);
...
if (pressed)
{
  GTPoint *myPoints = ts.readPoints();
...
void setup()
{
  ...
  uint8_t chkCalc = ts.calcChecksum((uint8_t *) cfg,  0x8100 - GT_REG_CFG  -1);
  uint8_t chkRead = ts.readChecksum();
